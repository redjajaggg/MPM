...
fetch(data_csv_normal).then(result=>result.text()).then(function (csvtext){return csv().fromString(csvtext);}).then(function(csv){readdomain(csv);});
fetch(data_csv_main).then(result=>result.text()).then(function (csvtext){return csv().fromString(csvtext);}).then(function(csv){mains = csv; main(csv); clicks()});
...
function c(){
    try{
        ...
        if(common !== null){document.getElementById("filter").value = common; filter();}
        if(nation !== null){document.getElementById("nationst").value = nation; nationst();}
        if(rate !== null){document.getElementById("rates").value = rate; rates();}
        if(mark !== null){document.getElementById("marked").value = mark; marked();}
        if(overall !== null){document.getElementById("overall").value = overall; overall();}
        if(purchase !== null){document.getElementById("purchase").value = purchase; purchase();}
        if(genre !== null){document.getElementById("genre").value = genre; genre();}
        if(read !== null){document.getElementById("read").value = read; read();}
