...
function buildImage(chapterName){
    ...
    //count number of img per chapter to find loaded error
    var countImg = 0;
    for(var i = 9; i < datas.length; i++){
        ...
        if(datas[i].Topic == chapterName){
            ...
            img.addEventListener("error", reloadImage(this));//if error
...
function layout(){
    if(document.getElementById("layout").value == "ver"){
        document.getElementById("show").innerHTML = "";
        document.getElementById("horNevigate").style.display = "none";
        document.getElementById("show").style.fontSize = "0px";
        start();
    }
    if(document.getElementById("layout").value == "horver"){
        document.getElementById("show").innerHTML = "";
        document.getElementById("horNevigate").style.display = "none";
        document.getElementById("show").style.fontSize = "30px";
        start();
    }
    if(document.getElementById("layout").value == "hor"){
        ...
        document.getElementById("horNevigate").style.display = "block";
    }
}
...
function goto(){
    ...
    fetch(`https://script.google.com/macros/s/AKfycbwcDe7zIvjY5p4rVQ-4mBIw3OCnI_ntnHhcKhmjGMGAgbB3ghnZ6MhVecVS1uHKWxD1Gw/exec?nm=${document.getElementById("nameget").innerText}&bm=${document.getElementById("allchapters").value}&tp=auto`)
}
//updateUrlParam
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleContent() {
        ...
        // If the count is odd, hide the content, else show it
        if (clickCount % 2 !== 0) {
            content.style.display = 'none'; // Hide content on odd clicks
        } else {
            content.style.display = 'block'; // Show content on even clicks
        }
    }

    // Listen for both mouse clicks and touch events
    document.getElementById("show").addEventListener('click', toggleContent);
    document.getElementById("show").addEventListener('touch', toggleContent);
});
//reload img if error

function reloadImage(img) {
    
    // Modify the src attribute to trigger a reload
    img.src = img.src;
