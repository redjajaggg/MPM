...
function appenddata(data){
    ...
    three.innerHTML = "Part: " + data[i].Part + " Number: " + data[i].Number + " Date: <label id='tendays'>" + covertdate(data[i].Date) + "</label>";
    ...
    fetch(datamore).then(resultsi=>resultsi.text()).then(function (csvtextsi){return csv().fromString(csvtextsi);}).then(function(csvsi){
        ...
        types = types.trim();
        anames.innerHTML = "Associate: " + data[i].AssName + "  <img src='" + flagl + "' style='width: 30px; border-radius: 2px;'>" + rate;
        type.innerHTML = "Genre: " + types;
        ...
        if(data[i].Notificate == "c"){
            noti.innerHTML = "Notice: Completed!";
            noti.style.backgroundColor = "red";
            noti.style.borderRadius = "10px";
        }
