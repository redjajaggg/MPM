const data_csv_main = "https://docs.google.com/spreadsheets/d/1U0F2QEFHx1vDufEJhb4_Pg_li0QgQHp6JtPRzlKOAAE/export?format=csv";
...
function statistic_main(datamain){
    ...
    for(var i = 0; i < all_num; i++){
        ...
        if(datamain[i].Website == "TRUE"){offline++;}
        if(datamain[i].Status == "end"){endstatus++;}
        if(datamain[i].Date == "n"){unkdate++;}
        if(datamain[i].Timeline == "H"){hiscount++;}
...
function statistic_more(){
    ...
    const data_csv_mores = "https://docs.google.com/spreadsheets/d/1NQIZPYNPTEyphmzSXmeWkrJMy1kWvE5C4NE-u2LwCqY/export?format=csv";
    fetch(data_csv_mores).then(result=>result.text()).then(function (csvtext){return csv().fromString(csvtext);}).then(function(datamore){
        ...
        for(var i = 0; i < all_num; i++){
            ...
            if(datamore[i].Overall == "c"){comcount++;}
...
function create_up(data, i){
    ...
    if(getday == data[i].Date && data[i].Status !== "end" && data[i].Convey == "TRUE"){
        ...
        data_div.innerHTML = "Part: " + data[i].Part + "<br>Number: " + data[i].Number + "<br><a href='empire-library-physiology/" + data[i].Name + "'>(Detail)</a><br>" + link1 + link2 + link3 + link4;
    ...
    var checkday = getday - 1;
    if(checkday == 0){checkday = 7;}
    if(checkday == data[i].Date && data[i].Status != "end" && data[i].Convey == "TRUE"){
...
function staticsinfo(){
    process("Statistic of Manhtia<br>Calculate from exists data that contains in database and Allow to show.");
}
function todayup(){
    process("Today's update is expected story to be update today(10.00pm Kr time).<br>Expected " + count_today + " story(s).")
}
function yesterdayup(){
    process("Yesterday's update is expected story to be update yesterday(10.00pm Kr time).<br>Expected " + count_yesterday + " story(s).")
}
